...
import SendSurgereEmail from '../../react-email-starter/emails/send-surgere';
...
export default async function handler(req, res) {
  const body = req.body;
  const emailHtml = render(<SendSurgereEmail data={body} />);
  ...
  const createSendEmailCommand = (toAddress, fromAddress) => {
    return new SendEmailCommand({
      ...
      Message: {
        ...
        Subject: {
          Charset: 'UTF-8',
          Data: 'Surgere Workshop Submission Form',
        },
      },
      Source: fromAddress,
      ReplyToAddresses: [
