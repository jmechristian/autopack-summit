...
import SendMorrisetteEmail from '../../react-email-starter/emails/send-morrisette';
...
export default async function handler(req, res) {
  const body = req.body;
  const emailHtml = render(<SendMorrisetteEmail data={body} />);
  ...
  const createSendEmailCommand = (toAddress, fromAddress) => {
    return new SendEmailCommand({
      ...
      Message: {
        ...
        Subject: {
          Charset: 'UTF-8',
          Data: 'Morrisette Tour Submission Form',
        },
      },
      Source: fromAddress,
      ReplyToAddresses: [
